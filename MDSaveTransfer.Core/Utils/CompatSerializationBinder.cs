...
// Copyright (c) 2018 Sirenix IVS, thepbone
//
...
namespace MDSaveTransfer.Utils
{
    ...
    public class CompatSerializationBinder : TwoWaySerializationBinder
    {
        ...
        static CompatSerializationBinder()
        {
        ...
        public override string BindToName(Type type, DebugContext debugContext = null)
        {
            ...
            lock (TYPETONAME_LOCK)
            {
                ...
                {
                    var typeName = type.FullName + ", " + ((type.Assembly.GetName().Name == "System.Private.CoreLib") ? "mscorlib" : type.Assembly.GetName().Name);
                    ...
                    if (type.IsGenericType)
                    {
                        ...
                        {
                            result = result.Replace(ass.FullName, (ass.GetName().Name == "System.Private.CoreLib" ? "mscorlib" : ass.GetName().Name));
                        }
                    }
                    ...
                    if (type.FullName.Contains("InternalStringComparerStub") || type.FullName.Contains("Generic.GenericEqualityComparer"))
                    {
                        result = "System.Collections.Generic.InternalStringComparer, mscorlib";
                    }
        ...
        public override Type BindToType(string typeName, DebugContext debugContext = null)
        {
            ...
            lock (NAMETOTYPE_LOCK)
            {
                if (typeMap.TryGetValue(typeName, out result) == false)
                {
                    ...
                    if (result == null)
                    {
                        if (typeName.Contains("System.Collections.Generic.InternalStringCompare"))
                            result = typeof(InternalStringComparerStub);
                        else
                            Console.Error.WriteLine("Failed deserialization type lookup for type name '" + typeName + "'.");
                    }
